...
def PlotCorrelations(corr1,corr2,cortype,season):
    ...
    if cortype=='mcor':
        fig.suptitle('Average product moment correlation over stations and models for '+season)
        ax1.set_title('rcp45')
        ax2.set_title('rcp85')
    
    if cortype=='rcor':
        fig.suptitle('Average rank correlation over stations and models for '+season)
        ax1.set_title('rcp45')
        ax2.set_title('rcp85')
    plt.show()
    pass
...
for i in range(4):
    PlotCorrelations(seasons_correlations[i][0],seasons_correlations[i][1],'mcor',Seasons[i])
    PlotCorrelations(seasons_correlations[i][2],seasons_correlations[i][3],'rcor',Seasons[i])
